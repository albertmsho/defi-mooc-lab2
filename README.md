...
## Exercise
In this exercise, I implemented a smart contract that performs a flash loan based liquidation.

### Prerequisite
- register an account on https://www.alchemy.com/ for access to an archive Ethereum node.

- Prepare the nodeJS environment for the project yourself, or have [docker](https://www.docker.com/) installed on your machine.

### Requirements

- The smart contract allows to perform a flash loan, a liquidation, and an asset exchange in one blockchain transaction.
```
...
By calling this function, the contract then repay some amount of token D to Aave and in return, some token C is sent to your account.

The contract makes sure that the user is in a liquidatable state. Otherwise, the aave smart contract would revert my transaction and you would pay transaction fees for an unsuccessful liquidation. 
...
Back to liquidation, I programed the liquidation logic in the `uniswapV2Call` function. So, when I don't have enough token D to perform a liquidation, I can request a flash loan and the smart contract can do the liquidation after receiving token D.

### What do you need to do after liquidation?
With the flash loan, we now have enough token D. You can repay the debt for the borrowing position and claim the collateral token C. Congratulation! A successful liquidation is completed, but, wait, you still need to repay the flash loan. Remember that you need to preserve the `K`. In the exercise, you are required to convert every earned token to ETH through e.g., exchanges. This is for easing the grading.
